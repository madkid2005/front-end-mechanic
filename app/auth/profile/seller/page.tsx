...
export default function SellerProfile() {
  ...
  const [formData, setFormData] = useState({
    username: '', national_code: '', first_name: '', last_name: '', email: '', shaba_number: '', gender: '', address: '', password: '',
  });
  const router = useRouter();

  useEffect(() => {
    if (user?.profile_completed) router.push('/products');
    if (user?.role !== 'seller') router.push(`/auth/profile/${user?.role}`);
  }, [user, router]);
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <input type="text" value={formData.shaba_number} onChange={e => setFormData({ ...formData, shaba_number: e.target.value })} placeholder="Shaba Number (Required)" className="w-full p-2 mb-2 border rounded" />
