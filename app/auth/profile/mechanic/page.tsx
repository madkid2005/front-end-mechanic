...
export default function MechanicProfile() {
  ...
  const [formData, setFormData] = useState({
    username: '', national_code: '', first_name: '', last_name: '', email: '', gender: '', address: '', password: '', has_mechanic_shop: false,
  });
  const router = useRouter();

  useEffect(() => {
    if (user?.profile_completed) router.push('/products');
    if (user?.role !== 'mechanic') router.push(`/auth/profile/${user?.role}`);
  }, [user, router]);
  ...
  return (
    <div className="max-w-md mx-auto">
      <h1 className="text-2xl font-bold mb-4">Complete Your Mechanic Profile</h1>
