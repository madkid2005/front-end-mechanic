...
export default function MechanicRegister() {
  ...
  const handleRequestTotp = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await authApi.post('/register/', { phone_number: phoneNumber, role: 'mechanic' });
      setStep(2);
    } catch (error) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await authApi.post('/register/', { phone_number: phoneNumber, totp_code: totpCode, role: 'mechanic' });
      const { access, refresh } = response.data;
      setAuthToken(access);
      const profileResponse = await authApi.get('/profile/');
      dispatch(setAuth({ token: access, user: profileResponse.data }));
      router.push('/auth/profile/mechanic');
    } catch (error) {
  ...
  return (
    <div className="max-w-md mx-auto">
      <h1 className="text-2xl font-bold mb-4">Register as Mechanic</h1>
      {step === 1 ? (
